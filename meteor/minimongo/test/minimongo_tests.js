var minimongo = require('meteor/minimongo')
var Minimongo = minimongo.Minimongo
var LocalCollection = minimongo.LocalCollection
var MinimongoTest = minimongo.MinimongoTest
...
it("minimongo - basics", function (done) {
  ...
  test.lengthOf(c.find({type: "kitten"}).fetch(), 2);
  test.lengthOf(c.find({type: "cryptographer"}).fetch(), 3);
  ...
  test.lengthOf(c.find({type: "kitten"}).fetch(), 2);
  test.lengthOf(c.find({type: "cryptographer"}).fetch(), 2);
  ...
  test.equal(c.find({type: "cryptographer"}).count(), 3);
  test.lengthOf(c.find({type: "kitten"}).fetch(), 1);
  test.lengthOf(c.find({type: "cryptographer"}).fetch(), 3);
  ...
  test.lengthOf(c.find({tags: "flower"}).fetch(), 1);
  test.lengthOf(c.find({tags: "fruit"}).fetch(), 2);
  test.lengthOf(c.find({tags: "red"}).fetch(), 3);
...
it("minimongo - cursors", function (done) {
  var c = new LocalCollection();
  var res;
  ...
  // fetch
  res = q.fetch();
  test.lengthOf(res, 20);
  for (var i = 0; i < 20; i++)
    test.equal(res[i].i, i);
  // everything empty
  test.lengthOf(q.fetch(), 0);
  q.rewind();
...
it("minimongo - transform", function (done) {
  var c = new LocalCollection;
  c.insert({});
...
it("minimongo - misc", function (done) {
  // deepcopy
...
it("minimongo - lookup", function (done) {
...
it("minimongo - selector_compiler", function (done) {
...
it("minimongo - projection_compiler", function (done) {
...
it("minimongo - fetch with fields", function (done) {
  var c = new LocalCollection();
...
it("minimongo - fetch with projection, subarrays", function (done) {
...
it("minimongo - fetch with projection, deep copy", function (done) {
...
it("minimongo - observe ordered with projection", function (done) {
...
it("minimongo - ordering", function (done) {
...
it("minimongo - sort", function (done) {
  var c = new LocalCollection();
  for (var i = 0; i < 50; i++)
    for (var j = 0; j < 2; j++)
...
it("minimongo - subkey sort", function (done) {
  var c = new LocalCollection();
...
it("minimongo - array sort", function (done) {
  var c = new LocalCollection();
...
it("minimongo - sort keys", function (done) {
...
it("minimongo - sort key filter", function (done) {
...
it("minimongo - binary search", function (done) {
...
it("minimongo - modify", function (done) {
...
it("minimongo - observe ordered", function (done) {
  var operations = [];
  var cbs = log_callbacks(operations);
  var handle;
...
_.each([true, false], function (ordered) {
  it("minimongo - observe ordered: " + ordered, function (done) {
    var c = new LocalCollection();
...
it("minimongo - diff changes ordering", function (done) {
...
it("minimongo - diff", function (done) {
...
it("minimongo - saveOriginals", function (done) {
  // set up some data
  var c = new LocalCollection(),
      count;
  ...
  test.ok(originals.has('temp'));
  test.equal(originals.get('temp'), undefined);
  done()
});

it("minimongo - saveOriginals errors", function (done) {
  var c = new LocalCollection();
...
it("minimongo - objectid transformation", function (done) {
...
it("minimongo - objectid", function (done) {
  var randomOid = new LocalCollection._ObjectID();
...
it("minimongo - pause", function (done) {
  var operations = [];
  var cbs = log_callbacks(operations);
...
it("minimongo - ids matched by selector", function (done) {
...
it.skip("minimongo - reactive stop", function (done) {
  var coll = new LocalCollection();
  coll.insert({_id: 'A'});
  coll.insert({_id: 'B'});
  coll.insert({_id: 'C'});
...
it("minimongo - immediate invalidate", function (done) {
  var coll = new LocalCollection();
  coll.insert({_id: 'A'});
...
it("minimongo - count on cursor with limit", function(done){
...
it("minimongo - $near operator tests", function (done) {
  var coll = new LocalCollection();
  ...
  _.each(points, function (point, i, points) {
    test.ok(!i || distance([0, 0], point.rest.loc) >= distance([0, 0], points[i - 1].rest.loc));
  });
